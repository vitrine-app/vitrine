...
import { getAppDataFolder, isProduction, isTesting } from '@models/env';
...
export class Logger {
  ...
  public createLogger(testEnv?: boolean) {
    ...
    const initialLog: string = `<style>
p { margin: 0 } span { font-family: monospace; background-color: lightgrey; width: 40px; display: inline-block; }
</style>
<h3>Vitrine log</h3><p><span>${++this.linesNb}</span><strong>[ ${dateTime} ]</strong> Starting logging.
</p>`;
  ...
  public info(channelName: string, message: any, displayed?: boolean) {
    ...
    if (displayed || (isProduction() && !isTesting())) {
