...
class BattleNetCrawler {
  ...
  public async search(battleNetModuleConfig: any) {
    this.moduleConfig = battleNetModuleConfig;

    const configFilePath: string = path.resolve(battleNetModuleConfig.configFilePath.replace('%appdata%', process.env.APPDATA));
    ...
    try {
      ...
      const gameTags: string[] = Object.keys(battleNetConfig.Games).filter(
        (gamesTag: string) => gamesTag !== 'battle_net' && battleNetConfig.Games[gamesTag].Resumable
      );
      const gamesData: BattleNetGame[] = gameTags
        .map((gameTag: string) => this.moduleConfig.gamesData.filter((battleNetGame: BattleNetGame) => battleNetGame.tag === gameTag)[0])
      ...
      const potentialGames: PotentialGame[] = (await Promise.all(
        gamesData.filter(async (gameData: BattleNetGame) => await fs.pathExists(path.resolve(this.rootInstallPath, gameData.name, gameData.path)))
