...
class OriginCrawler {
  ...
  public async search(originConfig: any) {
    ...
    this.regDetails = await Promise.all(
      items.map(async (key: Winreg.Registry) => {
  ...
  private async parseFolders(files: string[]) {
    const gameInfos: any[] = files
      .map((gameFolder: string) => ({
        gameFolder,
        gameName: gameFolder.split('/').pop()
      }))
      .filter(({ gameName }: any) => {
        const found: boolean = this.playableGames.filter((playableGame: any) => spatStr(gameName) === spatStr(playableGame.name)).length > 0;
        if (found) {
